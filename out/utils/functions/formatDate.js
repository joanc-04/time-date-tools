...
async function formatDate(time, options) {
    if (typeof (time) != "number")
        return exports_1.Console.error('error');
    for (let i of Object.entries(formatDateOptionsDefault))
        if (options?.[i[0]] == undefined)
            options[i[0]] = i[1];
    const date = new Date(time);
    if (!date)
        return exports_1.Console.error('Time is invalid');
    const textLanguage = exports_1.client.lang[options.lang || formatDateOptionsDefault.lang]?.dateKeys;
    const obj = {
        date: date,
