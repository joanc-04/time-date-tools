...
function formatTime(time, options) {
    ...
    if (isNaN(time))
        return exports_1.Console.error(message_json_1.default.ERROR_TIME_TYPE_NOT_A_NUMBER);
    ...
    for (let i of Object.entries(formatTimeOptionsDefault))
        if (options[i[0]] == undefined)
            options[i[0]] = i[1];
    const textLanguage = exports_1.client.lang[options.lang || formatTimeOptionsDefault.lang]?.timeKeys;
    const obj = {
        y: splitTime[0],
        mo: splitTime[1],
        d: splitTime[2],
    ...
    if (typeof (format) != 'string')
        ...
        format = format.trim();
    if (!['undefined', 'boolean'].includes(typeof (options.valueNull)))
        return exports_1.Console.error(message_json_1.default.ERROR_VALUE_NULL_TYPE_NOT_A_BOOLEAN);
    const regex = /YY|MMOO|DD|HH|MMSS|MM|SS|yy|mmoo|dd|hh|mmss|mm|ss|Y|MO|D|H|MS|M|S|y|mo|d|h|ms|m|s/, separators = format.split(regex).slice(1);
    let timeKeys = [], values = [], null_ = false;
    for (let i = 0; i < separators.length; i++) {
        ...
        if (options.valueNull && separator.includes('['))
            null_ = false;
        if (options.valueNull && separator.includes(']'))
            null_ = true;
    }
